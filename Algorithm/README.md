...
### 그래프 탐색    
  ...
  - __BFS(Breadth First Search)__   
    ...
    - 인접 행렬로 구현한 시간복잡도(V^2)   
      - 모든 정점 방문 O(V) * 각 정점 방문할 때마다 V개의 정점 모두 방문 O(V)   
    - 인접 리스트로 구현한 시간복잡도(V+E)   
      - 모든 정점 한 번씩만 방문 O(V) + 모든 간선 한번 검사 O(E)   
...
### Bellman-Ford    
  - __Bellman-Ford__   
    - __순서__    
      - 간선 1개를 이용해서 만들 수 있는 최단 거리를 구한다.__   
      - 간선 2개를 이용해서 만들 수 있는 최단 거리를 구한다.__   
      - ....   
      - 간선 V-1개를 이용해서 만들 수 있는 최단 거리를 구한다.__   
      - __즉, 정점 V개 있을 경우 V-1번 반복하여 최단 거리를 구한다.__   
      
    - __dist[i] = 시작점에서 i까지의 최단 경로__   
    - __모든 간선 e (from, to, cost)에 대해서 dist[to] = min(dist[to], dist[from] + cost)를 검사한다.__   
    - __시간복잡도 O(V * E)지만, E <= V^2이기 때문에 O(V^3)이다.__   
    - __가중치가 음수가 있는 경우에도 사용할 수 있다.__   
      - __음수 있을 경우, 음수 사이클 존재 시 최단 거리 구하는 것이 불가능하므로 음수 사이클 존재 검사가 필요하다.__   
    - __음수 사이클 확인 방법__   
      - __V번 최단 거리를 구해보면 된다__   
        - __V-1까지 한 결과와 V까지 한 결과가 다를 경우 최단 거리는 존재하지 않는다__   
          - 최단 거리는 항상 V-1개 이하의 간선으로 이루어져야 하기 때문이다.   
          - 최단 거리가 존재하다면, 100번을 더하던 값이 V-1개까지 한 경우와 같아야 한다.   
  ...
  - __이분 탐색__   
    - __정렬되어 있는 리스트에서 어떤 값을 빠르게 찾는 알고리즘__   
      ...
      - ex) A에서 B까지 X라는 시간으로 이동할 수 있나?   
        ...
        - __A에서 B로 이동하는 가장 빠른 시간은 3이다.__   
