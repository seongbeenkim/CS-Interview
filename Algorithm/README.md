...
### KMP(Knuth, Morris, Prett)      
  - __문자열 S에서 문자열 P를 찾을 때 사용하는 알고리즘__        
    - __주어진 문자열 P는 자신의 부분 문자열 패턴을 알아내기 위해 배열을 이용한다.__   
      - __pi[i] = 문자열 P의 i까지 부분 문자열에서 prefix(접두사) == suffix(접미사)가 될 수 있는 부분 문자열 중 가장 긴 것의 길이를 저장__     
      - __P에 대한 부분 문자열 검사하는데 걸리는 시간복잡도 = O(P)__   
    - __배열 pi를 이용하여 문자열 S와 문자열 P를 비교한다.__   
      - __최종 시간복잡도 = O(S+P)__   
